      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 space-y-12">
        {DOCTORS_DATA.map((doc, idx) => (
          <motion.div
            ...
            viewport={{ once: true }}
            className={`bg-white rounded-3xl border border-clinic-border shadow-md overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch ${
              idx % 2 === 1 ? 'lg:bg-clinic-section/40' : ''
            }`}
          >
            {/* Image Column */}
            <div className="lg:col-span-5 relative min-h-[360px] lg:min-h-[440px] bg-clinic-section overflow-hidden">
              ...
              />
              <div className="absolute top-4 left-4 bg-primary text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-md border border-white/20">
                {doc.experience}
              </div>

              {doc.college && (
                <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-md p-3 rounded-2xl border border-clinic-border text-xs text-clinic-dark shadow-lg">
            ...
            <div className="lg:col-span-7 p-6 sm:p-10 space-y-6 flex flex-col justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {doc.socials?.phone && (
                      <a
                        ...
                        className="w-8 h-8 rounded-full bg-clinic-section hover:bg-primary hover:text-white text-clinic-dark flex items-center justify-center transition-colors border border-clinic-border"
                    ...
                    {doc.socials?.email && (
                      <a
                        ...
                        className="w-8 h-8 rounded-full bg-clinic-section hover:bg-secondary hover:text-white text-clinic-dark flex items-center justify-center transition-colors border border-clinic-border"
                        title="Email Doctor"
                      >
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {doc.qualifications.map((q, i) => (
                      <span
                        key={i}
                        className="px-3 py-1 rounded-xl bg-primary-50 text-primary font-semibold text-xs border border-primary-100 shadow-sm"
                      >
                        {q}
                      </span>
                    ))}
              ...
              <div className="pt-4 border-t border-clinic-border flex items-center justify-between">
                <button
                  ...
                  className="bg-primary hover:bg-primary-dark text-white font-semibold py-3 px-6 rounded-xl shadow-md text-xs sm:text-sm flex items-center gap-2 transition-transform active:scale-95"
                >
